...
def main():
    ...
    base_dir = os.getcwd()
    cascade_dir = os.path.join(base_dir,"{}/{}".format('vehicle','india_license_plate.xml'))

    plate_cascade =cv2.CascadeClassifier(cascade_dir) # Loads the data required for detecting the license plates from cascade classifier.
    ...
    def detect_plate(img): # the function detects and perfors blurring on the number plate.
        ...
        for (x,y,w,h) in plate_rect:
            ...
            cv2.imwrite('buffer_img.jpg',plate_img[y:y+h, x:x+w, :])
    ...
    while True:
        ...
        if cv2.waitKey(1) == ord('s'):
            break  # press 'Esc' key to exit anytime.
